...
```{python}
#| label: structural-validation-shape 
#| code-fold: true
...
#shape
rows, cols = df.shape
print(f"The model dataset contains {rows} observations and {cols} variables.")
```

```{python}
#| label: structural-validation-check
#| echo: false
#| output: false
...
```{python}
#| label: overall-and-main-distribution-check
#| code-fold: true
#| echo: false
#| output: true

#need to fix rendering to look nicer 
#overall distribution check 
overall_dist = df.describe().round(3)

#distribution check for core features
core_dist = df[['alt','ast','bmi','homocysteine']].describe().round(3)
...
# check missingness
missing_dist = df.isna().sum().to_frame(name="missing_count")

#cant sort Nonetypes - need to convert 
# convert missing_count to integer
missing_dist["missing_count"] = missing_dist["missing_count"].astype(int)

#too long when rendered - need to prioritize/sort
missing_dist = missing_dist.sort_values(
    by="missing_count",
    ascending=False
)
...
#found error in R data integrity check that leads back here -need to address misclassified NAs 
df['NAFLD'] = np.where(df['HSI'].isna(), np.nan, (df['HSI'] >= 36).astype(int))

#three-level interpretive category (used downstream)
df['HSI_category'] = np.where(
  df['HSI'].isna(), np.nan, 
  np.where(df["HSI"] < 30, "healthy",
...
The crosstab confirms a clean mapping between HSI_category and NAFLD label. The 'borderline' bin is kept along with the binary NAFLD label since the 30-36 value range represents a potentially clinically meaningful subset of the population that is collapsed during binary classification. This 'borderline' bin allows downstream analyses to 
examine biomarkers across edge cases without affecting the binary labels used for model training. 
...
```{python}
#| label: sample-records-sum
#| code-fold: true
#| echo: false
#| output: true

#needd to render better - format same approach
pd.set_option('display.max_columns', None)
pd.set_option('display.width', 1000)

# 1. Display first few rows
print("\n1. Sample records:\n")
sample = df.head().round(2)
print(sample)
print("\n")
...
#HSI category counts 
df['HSI_category'].value_counts().to_frame(name="count")

#ML label balance 
#print(df['NAFLD'].value_counts())
df['NAFLD'].value_counts().to_frame(name="count")

#cross check bins 
#print(pd.crosstab(df['HSI_category'], df['NAFLD']))
pd.crosstab(df['HSI_category'], df['NAFLD'])
```
...
```{python}
#| label: display-models-shapes
#| code-fold: true
#| echo: false
#| output: true

#quick sanity check of inputs
#needed to be reworked for rendeing 
feature_summary = pd.DataFrame({
    "Model": ["Core", "Biomarker"],
    "Rows": [X_core.shape[0], X_bio.shape[0]],
    "Features": [X_core.shape[1], X_bio.shape[1]]
})
...
```{python}
#| label: display-model-class-imbalances
#| code-fold: true
#| echo: false
#| output: true

#now check class balances 
core_class = Y_core.value_counts()
bio_class = Y_bio.value_counts()

#Also need to compensate for pandas formatting 
print("Baseline class balance:")
display(core_class.to_frame(name="count"))

print("\nBiomarker class balance:")
display(bio_class.to_frame(name="count"))
```
...
```{python}
#| label: display-core-class-balance
#| code-fold: true
#| echo: true
#| output: true
...
output = (
    ...
    f"  Test set:     {X_core_test.shape[0]} samples, {X_core_test.shape[1]} features"
)
...
```{python}
#| label: display-bio-class-balance
#| code-fold: true
#| echo: true
#| output: true
...
bio_output = (
    ...
    f"  Test set:     {X_bio_test.shape[0]} samples, {X_bio_test.shape[1]} features"
)
...
plt.title("Core Class Balance across Training and Test sets (NAFLD vs Healthy)", pad=15)
plt.ylabel("Number of participants")
plt.xlabel("")
plt.legend(title="NAFLD label (0 = healthy, 1 = NAFLD)")
...
```{python}
#| label: model-accuracy-comparison
#| echo: false
#| output: true
...
```{python}
#| label: logisitic-regression-equation
#| echo: true
#| code-fold: true
#| output: true
...
#reformat for image purposes
equation = f"logit(P(NAFLD)) = {bio_intercept:.6f}"
...
```{python}
#| label: multicollinearity-heatmap
#| code-fold: true
#| echo: true
#| output: true
...
#fix tic labels 
#for rendering - need temp values not to print
...
```{python}
#| label: bimodal-sex-distribution
#| code-fold: true
#| echo: false
#| output: true
...
display(pd.Series(X_core_train['female'])
  .value_counts(normalize=True)
  .mul(100)
  .round(1)
  .astype(str)
...
display(pd.Series(X_bio_train['female'])
  .value_counts(normalize=True)
  .mul(100)
  .round(1)
  .astype(str)
...
#rework for rendering -same output approach as before
result = bio_df.groupby("female")["NAFLD"].mean().round(3)
result_table = result.to_frame(name="NAFLD prevalence")
result_table.index = ["Male (0)", "Female (1)"]

print("NAFLD prevalence by sex (biomarker dataset):")
display(result_table)
```

```{python}
#| label: female-proportion-distribution
#| echo: false
#| output: true

#set variables for rendering 
core_f_prop = X_core_train['female'].mean().round(5)
bio_f_prop = X_bio_train['female'].mean().round(5)

#testing against missingness bias across models 
print(f"Proportion female in full dataset: {core_f_prop}")
#print(X_core_train['female'].mean().round(5))

print(f"Proportion female in biomarker subset: {bio_f_prop}")
...
```{python}
#| label: biomarker-effect-multiplot
#| echo: true
#| output: true
#| code-fold: true
...
_ = fig.text(0.003, 0.5, 'P(NAFLD)', va='center', rotation='vertical', fontsize=10)

#now add global legend 
_ = fig.legend(loc='upper center', bbox_to_anchor=(0.5, 1.02), ncol=2, fontsize=9)
