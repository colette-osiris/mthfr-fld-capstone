...
#### <u> 2.1. Database Preprocessing Overview </u>
...
Cleaning and preprocessing were preformed in the staging schema using the files:
...
Despite efforts to use all TEXT formatting in CSV conversion, integer variables were automatically converted to floats during the .XPT to CSV conversion process. A nested recasting approach was used to correct this in which all affected variables were first cast from text to numeric. Then, when appropriate, variables were recast from numeric to integer data type; this ensured correct datatypes for downstream joins and analysis.
...
Cohorts were merged using LEFT JOIN operations across all relevant component tables, with the final cohort structure prioritizing maximal participant inclusion, preservation of missingness, and flexibility for downstream filtering in Python and R.
...
It's worth noting that cohort 2's ferritin displayed lower coverage than other biomarkers. In addition, differences in biomarker availability across cohorts were retained and accounted for in downstream analysis.
